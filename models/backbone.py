    ...
    def __init__(self, backbone_name: str, num_feature_levels: int, pretrained=None, use_checkpoint=False, dilation=False):
        if "dilation" in backbone_name:
            ...
            if 'small' in backbone_name:
                backbone = Dilation_Transformer(depths=[2, 2, 18, 2], dilation_rate_list=[1,2,3], out_indices=out_ind, use_checkpoint=use_checkpoint)
            elif 'base' in backbone_name:
                if "384" in backbone_name:
                    backbone = Dilation_Transformer(pretrain_img_size=384, depths=[2, 2, 18, 2], dilation_rate_list=[1,2,3], embed_dim=128, num_heads=[4, 8, 16, 32], out_indices=out_ind, window_size=12, use_checkpoint=use_checkpoint)
                else:
                    backbone = Dilation_Transformer(depths=[2, 2, 18, 2], dilation_rate_list=[1,2,3], embed_dim=128, num_heads=[4, 8, 16, 32], out_indices=out_ind, use_checkpoint=use_checkpoint)
            elif 'large' in backbone_name:
                if "384" in backbone_name:
                    backbone = Dilation_Transformer(pretrain_img_size=384, depths=[2, 2, 18, 2], dilation_rate_list=[1,2,3], embed_dim=192, num_heads=[6, 12, 24, 48], out_indices=out_ind, window_size=12, use_checkpoint=use_checkpoint)
                else:
                    backbone = Dilation_Transformer(depths=[2, 2, 18, 2], dilation_rate_list=[1,2,3], embed_dim=192, num_heads=[6, 12, 24, 48], out_indices=out_ind, use_checkpoint=use_checkpoint)
            else:
                backbone = Dilation_Transformer(depths=[2, 2, 6, 2], dilation_rate_list=[1,2,3], out_indices=out_ind, use_checkpoint=use_checkpoint)
        else:
...
def build_backbone(args):
    ...
    model.num_channels = backbone.num_channels[-1]
    return model
